...
zone_after_rebirth = 16  # Beardverse
zone_after_training = 18  # badly drawn world
...
deadline_adv_wandoos = time.strptime("01:40:00", "%H:%M:%S")  # deadline_augmentation + datetime.timedelta(minutes=15)
deadline_wandoos = time.strptime("02:00:00", "%H:%M:%S")  # deadline_adv_wandoos + datetime.timedelta(minutes=15)
deadline_gold = time.strptime("02:40:00", "%H:%M:%S")  # for wear gold drop equip for gold drop from titan
...
while True:
    ...
    if rt.days == 0 and rt.timestamp < deadline_before_adv_training:
        ...
        if curState != stateName:
            ...
            GoldDiggers.gold_diggers(targets=[2])  # wandoos
        ...
        if curState != stateName:
            ...
            Inventory.loadout(gold_loadout)
            GoldDiggers.deactivate_all_diggers()
            GoldDiggers.gold_diggers(targets=[4, 1], deactivate=True)  # add adv stats
        if Misc.get_idle_cap(1) > 0 or Misc.get_idle_cap(2) > 0:
            TimeMachine.time_machine(e=Misc.get_idle_cap(1),
        ...
        if curState != stateName:
            ...
            Inventory.loadout(ngu_loadout)
            GoldDiggers.deactivate_all_diggers()
            GoldDiggers.gold_diggers([5, 6, 4, 1], deactivate=True)  # Energy NGU, Magic NGU, Adventure Stat, drop
        ...
        #Adventure.snipe(zone=zone_after_training, duration=1, once=True, bosses=True, manual=True)
        Adventure.itopod_snipe(60)
        #Adventure.snipe(zone=zone_after_training, duration=1, once=False, bosses=True, manual=True)
